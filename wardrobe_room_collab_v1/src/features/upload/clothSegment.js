...
// 做法：以图像四边采样出背景参考色，从边界做泛洪，只走"与背景色相近"的像素，
// 模型 mask 作为不可穿越的屏障。泛洪到不了的、且颜色明显不是背景的像素，
// 就是被衣物轮廓包住的"漏检衣物区"（如裤腰），补进前景；
// 被轮廓包住但仍是背景色的区域（如袖窿/腋下空隙、领口洞）保持透明。
const BG_COLOR_DIST = 48; // RGB 欧氏距离阈值：< 视为背景色
function refineMaskByBackgroundFlood(ctx, labels, chosen) {
  const S = INPUT_SIZE;
  const n = S * S;
  const { data } = ctx.getImageData(0, 0, S, S);

  const seed = new Uint8Array(n);
  for (let i = 0; i < n; i += 1) if (labels[i] === chosen) seed[i] = 1;
  ...
  // 从四边泛洪：只能走 bgLike，且不能穿过模型前景
  const reached = new Uint8Array(n);
  const stack = [];
  const push = i => {
    if (!reached[i] && !seed[i] && bgLike[i]) { reached[i] = 1; stack.push(i); }
  };
  ...
  // 前景 = 模型种子 ∪（泛洪未到达 且 颜色非背景色）
  const out = new Uint8Array(n);
  for (let i = 0; i < n; i += 1) {
    out[i] = seed[i] || (!reached[i] && !bgLike[i]) ? 1 : 0;
  }
...
export async function segmentCloth(blob, category, onProgress) {
  ...
  // 模型漏检修补：背景泛洪，把被衣物轮廓包住却被判成背景的区域（如裤腰）补进前景；
  // 袖窿/领口等"包住但仍是背景色"的区域保持透明
  const refined = refineMaskByBackgroundFlood(inputCtx, labels, chosen);
